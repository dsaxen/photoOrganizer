...
public class SlideShowWindow extends JFrame implements WindowListener {
	...
	public SlideShowWindow(AlbumInterface a) {
		root = a;
		init();
	};
	...
	public void init() {
		...
		view.setText(root.toString());//set text
		...
		// setup the timer
		timer.scheduleAtFixedRate(new TimerTask() {//tick will be ran every 40 ms
			public void run() {
				tick();
			}
		}, MSPERTICK, MSPERTICK);
	...
	public void loadPhotos(String path) {
		...
		while(iterator.hasNext()) {
			((Album) root).addPhoto(iterator.next()); //nu har rootAlbum alla foton fr�n samples-photos foldern
		}
	}
