...
public class Thumbnail extends JComponent {
	...
	public void paintComponent(Graphics g) {
		...
		if (isSelected()) {
			g2.setColor(new Color(0.0f /* red */, 1.0f /* green */,
					0.0f /* blue */, 0.5f /* alpha (transparency) */));
			g2.fillRect(x, y, width, height);
			repaint();
		}
		
		if (getPhoto().getFlagged()==true){
			super.paintComponent(g);
			g2.setColor(new Color(1.0f /* red */, 0.0f /* green */,
					0.0f /* blue */, 0.5f /* alpha (transparency) */));
			g2.fillRect(x,y, d.width/7, d.height/7);
			repaint();
			invalidate();
		}
		if (getPhoto().getRating()>0){
			super.paintComponent(g);
			if (getPhoto().getRating()>=4){
			...
			g2.setFont( font );
			g2.drawString("Rating:" +getPhoto().getRating(), 0, 40);
			repaint();
			invalidate();
		}
	}
